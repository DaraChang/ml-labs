...
class WeatherTTADataset(Dataset):
    def __init__(self,labels_file,aug):
        ...
        for index, row in labels_file.iterrows():
            imgs.append((row["image_id"],row["label"]))
        self.imgs = imgs
...
with torch.no_grad():
    ...
    #print(predicted.data.cpu().numpy().tolist())
    ...
    for i in range(len(predicted)):
       if predicted[i]==1:
           ...
           predicted[i]='C'
    test_files.label = predicted
    test_files.to_csv('./submits/%s_baseline.csv' % configs.model_name, index=False)
   #with open('./submits/se_resnext50_32x4d-model-sgd-512_baseline.csv', 'w') as f:
   #    csv_writer = writer(f)
