...
def get_files(root,mode):
    if mode == "test":
        ...
        image_folders.sort()
        #all_images = list(chain.from_iterable(list(image_folders)))
        #all_images = list(chain.from_iterable(list(map(lambda x: glob(x + "/*"), image_folders))))
        #print(all_images)
        if(root.split('/')[2]=='C1-P1_Train'):
            df = pd.read_csv('./C1-P1_Train Dev_fixed/train.csv')
        elif(root.split('/')[2]=='C1-P1_Dev'):
            df = pd.read_csv('./C1-P1_Train Dev_fixed/dev.csv')
        #print(df)
